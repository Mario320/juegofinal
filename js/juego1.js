...
function preload() {
	game.load.image('fondo', 'assets/fondo1.jpg');
    game.load.image('plataforma', 'assets/platform.png');
    game.load.image('cielo', 'assets/totototo.png');
    game.load.image('diamante', 'assets/diamond.png');
    game.load.image('bomba', 'assets/bomb.png');
    game.load.image('batman', 'assets/batmancaminando.gif');
    game.load.spritesheet('personaje', 'assets/dude.png', 32, 48);
    game.load.spritesheet('joker', 'assets/joker.png', 43, 68);
    game.load.audio('punto1', 'assets/lol.mp3');
...
function create() {
	...
	plataforma = game.add.sprite(0, 110, 'cielo');
	...
	bandeja = game.add.sprite(50, game.world.height - 150, 'batman');
	bandeja.width = 150;
	game.physics.arcade.enable(bandeja);
	
	personaje = game.add.sprite(32, 0, 'joker');
	game.physics.arcade.enable(personaje);
	personaje.body.gravity.y = 300;
	personaje.body.velocity.x = 250;
	
	personaje.animations.add('left', [0], 10, true);
    personaje.animations.add('right', [1], 10, true);
	...
	txtPuntaje = game.add.text(25, 16, 'Puntaje: 0', { font: '24px Arial', fill: '#FFFFFF' });
	txtNivel = game.add.text(325, 16, 'Nivel: 1', {font: '24px Arial', fill: '#FFFFFF'});
	txtVidas = game.add.text(625, 16, 'Vidas: 5', {font: '24px Arial', fill: '#FFFFFF'});
...
function soltarDiamante() {
    var diamante = diamantes.create(personaje.x, 100, 'bomba');
